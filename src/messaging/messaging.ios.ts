...
const NOTIFICATIONS_REGISTRATION_KEY =
  "Firebase-RegisterForRemoteNotifications";
...
export function initFirebaseMessaging(options) {
  ...
  _showNotifications =
    options.showNotifications === undefined
      ? _showNotifications
  ...
  _autoClearBadge =
    options.autoClearBadge === undefined
      ? _autoClearBadge
      : !!options.autoClearBadge;
...
export function addBackgroundRemoteNotificationHandler(appDelegate) {
  appDelegate.prototype.applicationDidRegisterForRemoteNotificationsWithDeviceToken = (
    ...
    if (areNotificationsEnabled()) {
      _resolveWhenDidRegisterForNotifications &&
        _resolveWhenDidRegisterForNotifications();
    } else {
  ...
  appDelegate.prototype.applicationDidFailToRegisterForRemoteNotificationsWithError = (
    ...
    if (
      error.localizedDescription.indexOf("not supported in the simulator") > -1
    ) {
  ...
  appDelegate.prototype.applicationDidReceiveRemoteNotificationFetchCompletionHandler = (
    ...
    if (typeof FIRAuth !== "undefined") {
      if (
        firebase._configured &&
        FIRAuth.auth().canHandleNotification(notification)
      ) {
...
export function registerForInteractivePush(
  model?: PushNotificationModel
): void {
  let nativeActions: Array<UNNotificationAction> = [];

  model.iosSettings.interactiveSettings.actions.forEach(
    (action: IosInteractiveNotificationAction) => {
  ...
  let actions: NSArray<UNNotificationAction> = <NSArray<UNNotificationAction>>(
    NSArray.arrayWithArray(<any>nativeActions)
  );
  let nativeCategories: Array<UNNotificationCategory> = [];

  model.iosSettings.interactiveSettings.categories.forEach(category => {
    let nativeCategory = UNNotificationCategory.categoryWithIdentifierActionsIntentIdentifiersOptions(
      category.identifier,
      actions,
      null,
      null
  ...
  const nsSetCategories = <NSSet<UNNotificationCategory>>(
    new NSSet(<any>nativeCategories)
  );
  ...
  if (model.onNotificationActionTakenCallback) {
    _addOnNotificationActionTakenCallback(
...
export function prepAppDelegate() {
  ...
  _addObserver("com.firebase.iid.notif.refresh-token", notification =>
    onTokenRefreshNotification(notification.object)
  );
...
export function subscribeToTopic(topicName) {
  return new Promise((resolve, reject) => {
    try {
      ...
      FIRMessaging.messaging().subscribeToTopicCompletion(
        topicName,
        (error: NSError) => {
...
export function unsubscribeFromTopic(topicName) {
  return new Promise((resolve, reject) => {
    try {
      ...
      FIRMessaging.messaging().unsubscribeFromTopicCompletion(
        topicName,
        error => {
          error ? reject(error.localizedDescription) : resolve();
        }
...
function _registerForRemoteNotifications(resolve?, reject?) {
  ...
  if (parseInt(device.osVersion) >= 10) {
    ...
    if (_showNotifications) {
      _userNotificationCenterDelegate = UNUserNotificationCenterDelegateImpl.new().initWithCallback(
    ...
    if (typeof FIRMessaging !== "undefined") {
      _firebaseRemoteMessageDelegate = FIRMessagingDelegateImpl.new().initWithCallback(
        (appDataDictionary: NSDictionary<any, any>) => {
          const userInfoJSON = firebaseUtils.toJsObject(appDataDictionary);
          updateUserInfo(userInfoJSON);
          _pendingNotifications.push(userInfoJSON);

          const asJs = firebaseUtils.toJsObject(
            appDataDictionary.objectForKey("notification")
          );
          if (asJs) {
    ...
    const notificationSettings = UIUserNotificationSettings.settingsForTypesCategories(
      notificationTypes,
      null
    );
    firebaseUtils.invokeOnRunLoop(() => {
...
function _addOnNotificationActionTakenCallback(callback: Function) {
  return new Promise((resolve, reject) => {
    try {
      ...
      console.log(
        "Error in messaging._addOnNotificationActionTakenCallback: " + ex
      );
      reject(ex);
    }
  });
...
class UNUserNotificationCenterDelegateImpl extends NSObject
  implements UNUserNotificationCenterDelegate {
  public static ObjCProtocols = [];

  static new(): UNUserNotificationCenterDelegateImpl {
    if (
      UNUserNotificationCenterDelegateImpl.ObjCProtocols.length === 0 &&
      typeof UNUserNotificationCenterDelegate !== "undefined"
    ) {
      UNUserNotificationCenterDelegateImpl.ObjCProtocols.push(
  ...
  ): UNUserNotificationCenterDelegateImpl {
...
class FIRMessagingDelegateImpl extends NSObject
  implements FIRMessagingDelegate {
  public static ObjCProtocols = [];

  static new(): FIRMessagingDelegateImpl {
    if (
      FIRMessagingDelegateImpl.ObjCProtocols.length === 0 &&
      typeof FIRMessagingDelegate !== "undefined"
    ) {
  ...
  public initWithCallback(
    callback: (appData: NSDictionary<any, any>) => void
  ): FIRMessagingDelegateImpl {
    this.callback = callback;
    return this;
  }

  public messagingDidReceiveMessage(
    messaging: FIRMessaging,
    remoteMessage: FIRMessagingRemoteMessage
  ): void {
    this.callback(remoteMessage.appData);
  }

  public messagingDidReceiveRegistrationToken(
    messaging: FIRMessaging,
    fcmToken: string
  ): void {
